...
static void sunxi_ir_recv(u32 reg_data)
{
	...
	if (pluse_pre == pluse_now) {
		...
		if (ir_data->ir_protocol_used == RC5) {
		...
		if (ir_data->ir_protocol_used == NEC) {
			if (is_receiving) {
				rawir.duration *= IR_SIMPLE_UNIT;
				dprintk(DEBUG_INT, "pusle :polar=%d, dur: %u ns\n",
						rawir.pulse, rawir.duration);
...
static void ir_reg_cfg(void)
{
	...
	if (ir_data->ir_protocol_used == RC5) {
		...
		ir_sample_config(IR_FILTER_TH_RC5); 	/* Set Filter Threshold */
	}
	if (ir_data->ir_protocol_used == NEC) {
		ir_sample_config(IR_ACTIVE_TH); 		/* Set Active Threshold */
		ir_sample_config(IR_FILTER_TH_NEC); 	/* Set Filter Threshold */
	}
...
static int sunxi_ir_startup(struct platform_device *pdev)
{
	...
	} else if (ir_data->ir_protocol_used ==  RC5) {
		for (i = 0; i < MAX_ADDR_NUM; i++) {
...
static int sunxi_ir_recv_resume(struct device *dev)
{
	...
	if (wakeup_event & CPUS_WAKEUP_HDMI_CEC) {
		pr_debug("wakeup source is hdmi_cec_standby\n");
		rc_keydown(sunxi_rcdev, (ir_data->ir_addr[0] << 8) | ir_data->ir_powerkey[0], 0);
		msleep(1);
		rc_keyup(sunxi_rcdev);
	}
