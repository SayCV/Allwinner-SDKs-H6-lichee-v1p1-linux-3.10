		...
		unsigned long ioctl_param)
{
	...
	case CHECK_SOC_BONDING:
		...
		if (copy_to_user((void __user *)ioctl_param, id, 8))
			ret = -EFAULT;
		pr_debug("soc id:%s\n", id);
		break;
	case CHECK_SOC_ROTPK:
#ifdef NO_EXPORT_PK_SUPPORT
		if (copy_from_user(rotpk, (void __user *)ioctl_param, 32))
			ret = -EFAULT;
		ret = sunxi_check_soc_rotpk((void *)rotpk, 32);
#else
		ret = sunxi_get_soc_rotpk((void *)rotpk, 32);
		if (ret > 0 &&
			copy_to_user((void __user *)ioctl_param, rotpk, 32))
			ret = -EFAULT;
#endif
		break;

	case CHECK_SOC_CUSTOMER_ID:
		customer_id = sunxi_get_soc_customerid();
		if (copy_to_user((void __user *)ioctl_param, &customer_id, 4))
			ret = -EFAULT;
		pr_debug("customer id is :%d\n", customer_id);
		break;

	case CHECK_SOC_MEDIA_ID:
		mediaid = sunxi_get_soc_mediaid();
		if (copy_to_user((void __user *)ioctl_param, &mediaid, 4))
			ret = -EFAULT;
		pr_debug("media id is :%d\n", mediaid);
		break;


	case SYS_EFUSE_GET_SN:
		ret = sunxi_get_soc_sn((void *)sn, 24);
		if (ret > 0 &&
			copy_to_user((void __user *)ioctl_param, sn, 24))
			ret = -EFAULT;
		break;
	default:
			     ...
			     struct class_attribute *attr, char *buf)
{
	...
	len = sunxi_efuse_readn_ext(key_name, key_data, 256*4);
	if (len <= 0) {
		vfree(key_data);
		return -EACCES;
	}
	size += sprintf(buf + size, "%s", key_name);
	for (i = 0; i < (len >> 2); i++) {
		if ((i > 0) && (i % 8 == 0))
