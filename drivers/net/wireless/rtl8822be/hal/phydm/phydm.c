...
)
{
	...
	phydm_adaptive_soml_init(p_dm_odm);
...
)
{
	...
	if (dm_value[0] == 100) {
		...
		PHYDM_SNPRINTF((output + used, out_len - used, "23. (( %s ))ADAPTIVE_SOML\n", ((p_dm_odm->support_ability & ODM_BB_ADAPTIVE_SOML) ? ("V") : ("."))));
		PHYDM_SNPRINTF((output + used, out_len - used, "%s\n", "================================"));
	}
	/*
...
)
{
	...
	phydm_adaptive_soml_timers(p_dm_odm, INIT_SOML_TIMMER);
...
)
{
	...
	phydm_adaptive_soml_timers(p_dm_odm, CANCEL_SOML_TIMMER);
...
)
{
	...
	phydm_adaptive_soml_timers(p_dm_odm, RELEASE_SOML_TIMMER);
