			       ...
			       u32 *val)
{
	...
	if (pp->ops->rd_own_conf)
		...
		ret = sunxi_pcie_cfg_read(pp->dbi_base + (where & ~0x3), where,
				size, val);
			       ...
			       u32 val)
{
	...
	if (pp->ops->wr_own_conf)
		...
		ret = sunxi_pcie_cfg_write(pp->dbi_base + (where & ~0x3), where,
				size, val);
...
irqreturn_t sunxi_handle_msi_irq(struct pcie_port *pp)
{
	...
	for (i = 0; i < MAX_MSI_CTRLS; i++) {
		sunxi_pcie_rd_own_conf(pp, PCIE_MSI_INTR0_STATUS + i * 12, 4,
				(u32 *)&val);
		if (val) {
			ret = IRQ_HANDLED;
			pos = 0;
			    ...
			    unsigned int nvec, unsigned int pos)
{
	...
	for (i = 0; i < nvec; i++) {
		...
		res = ((pos + i) / 32) * 12;
		bit = (pos + i) % 32;
		sunxi_pcie_rd_own_conf(pp, PCIE_MSI_INTR0_ENABLE + res, 4, &val);
		val &= ~(1 << bit);
		sunxi_pcie_wr_own_conf(pp, PCIE_MSI_INTR0_ENABLE + res, 4, val);
	}
}
...
static int assign_irq(int no_irqs, struct msi_desc *desc, int *pos)
{
	...
	/*
	 * irq_create_mapping (called from sunxi_pcie_host_init) pre-allocates
...
static int sunxi_msi_setup_irq(struct msi_chip *chip, struct pci_dev *pdev,
...
static void sunxi_msi_teardown_irq(struct msi_chip *chip, unsigned int irq)
{
	clear_irq(irq);
}

static struct msi_chip sunxi_pcie_msi_chip = {
...
int __init sunxi_pcie_host_init(struct pcie_port *pp)
{
	...
	for_each_of_pci_range(&parser, &range) {
		...
		/*	pp->config.cfg0_size = resource_size(&pp->cfg)/2; */
			pp->config.cfg0_size = resource_size(&pp->cfg);
...
*/
	...
	sunxi_pci.nr_controllers = 1;
	sunxi_pci.private_data = (void **)&pp;

	pci_common_init_dev(pp->dev, &sunxi_pci);
...
static void sunxi_pcie_prog_viewport_cfg0(struct pcie_port *pp, u32 busdev)
{
	...
	sunxi_pcie_writel_rc(pp, PCIE_ATU_REGION_OUTBOUND | PCIE_ATU_REGION_INDEX0,
			  PCIE_ATU_VIEWPORT);
	sunxi_pcie_writel_rc(pp, pp->cfg0_base, PCIE_ATU_LOWER_BASE);
	sunxi_pcie_writel_rc(pp, (pp->cfg0_base >> 32), PCIE_ATU_UPPER_BASE);
	sunxi_pcie_writel_rc(pp, pp->cfg0_base + pp->config.cfg0_size - 1,
			  PCIE_ATU_LIMIT);
	sunxi_pcie_writel_rc(pp, busdev, PCIE_ATU_LOWER_TARGET);
	sunxi_pcie_writel_rc(pp, 0, PCIE_ATU_UPPER_TARGET);
	sunxi_pcie_writel_rc(pp, PCIE_ATU_TYPE_CFG0, PCIE_ATU_CR1);
	sunxi_pcie_writel_rc(pp, PCIE_ATU_ENABLE, PCIE_ATU_CR2);
}

static void sunxi_pcie_prog_viewport_cfg1(struct pcie_port *pp, u32 busdev)
{
	/* Program viewport 1 : OUTBOUND : CFG1 */
	sunxi_pcie_writel_rc(pp, PCIE_ATU_REGION_OUTBOUND | PCIE_ATU_REGION_INDEX1,
			  PCIE_ATU_VIEWPORT);
	sunxi_pcie_writel_rc(pp, PCIE_ATU_TYPE_CFG1, PCIE_ATU_CR1);
	sunxi_pcie_writel_rc(pp, pp->cfg1_base, PCIE_ATU_LOWER_BASE);
	sunxi_pcie_writel_rc(pp, (pp->cfg1_base >> 32), PCIE_ATU_UPPER_BASE);
	sunxi_pcie_writel_rc(pp, pp->cfg1_base + pp->config.cfg1_size - 1,
			  PCIE_ATU_LIMIT);
	sunxi_pcie_writel_rc(pp, busdev, PCIE_ATU_LOWER_TARGET);
	sunxi_pcie_writel_rc(pp, 0, PCIE_ATU_UPPER_TARGET);
	sunxi_pcie_writel_rc(pp, PCIE_ATU_ENABLE, PCIE_ATU_CR2);
...
static void sunxi_pcie_prog_viewport_mem_outbound(struct pcie_port *pp)
{
	/* Program viewport 0 : OUTBOUND : MEM */
	sunxi_pcie_writel_rc(pp, PCIE_ATU_REGION_OUTBOUND | PCIE_ATU_REGION_INDEX0,
			  PCIE_ATU_VIEWPORT);
	sunxi_pcie_writel_rc(pp, PCIE_ATU_TYPE_MEM, PCIE_ATU_CR1);
	sunxi_pcie_writel_rc(pp, pp->mem_base, PCIE_ATU_LOWER_BASE);
	sunxi_pcie_writel_rc(pp, (pp->mem_base >> 32), PCIE_ATU_UPPER_BASE);
	sunxi_pcie_writel_rc(pp, pp->mem_base + pp->config.mem_size - 1,
			  PCIE_ATU_LIMIT);
	sunxi_pcie_writel_rc(pp, pp->config.mem_bus_addr, PCIE_ATU_LOWER_TARGET);
	sunxi_pcie_writel_rc(pp, upper_32_bits(pp->config.mem_bus_addr),
			  PCIE_ATU_UPPER_TARGET);
	sunxi_pcie_writel_rc(pp, PCIE_ATU_ENABLE, PCIE_ATU_CR2);
}

static void sunxi_pcie_prog_viewport_io_outbound(struct pcie_port *pp)
{
	/* Program viewport 1 : OUTBOUND : IO */
	sunxi_pcie_writel_rc(pp, PCIE_ATU_REGION_OUTBOUND | PCIE_ATU_REGION_INDEX1,
			  PCIE_ATU_VIEWPORT);
	sunxi_pcie_writel_rc(pp, PCIE_ATU_TYPE_IO, PCIE_ATU_CR1);
	sunxi_pcie_writel_rc(pp, pp->io_base, PCIE_ATU_LOWER_BASE);
	sunxi_pcie_writel_rc(pp, (pp->io_base >> 32), PCIE_ATU_UPPER_BASE);
	sunxi_pcie_writel_rc(pp, pp->io_base + pp->config.io_size - 1,
			  PCIE_ATU_LIMIT);
	sunxi_pcie_writel_rc(pp, pp->config.io_bus_addr, PCIE_ATU_LOWER_TARGET);
	sunxi_pcie_writel_rc(pp, upper_32_bits(pp->config.io_bus_addr),
			  PCIE_ATU_UPPER_TARGET);
	sunxi_pcie_writel_rc(pp, PCIE_ATU_ENABLE, PCIE_ATU_CR2);
}
		...
		u32 devfn, int where, int size, u32 *val)
{
	...
	address = where & PCIE_ADDRESS_ALIGNING;

	if (bus->parent->number == pp->root_bus_nr) {
#ifdef CONFIG_ARCH_SUN50IW6
		flags = sunxi_pcie_cutpage_spin_lock();
#endif
		sunxi_pcie_writel_rc(pp, (pp->cfg0_base >> PCIE_HIGH_16), PCIE_ADDR_PAGE_CFG);
		...
		sunxi_pcie_writel_rc(pp, (pcie_page >> PCIE_HIGH_16), PCIE_ADDR_PAGE_CFG);
...
static struct pci_bus *sunxi_pcie_scan_bus(int nr, struct pci_sys_data *sys)
{
	struct pci_bus *bus;
	...
	if (pp) {
		...
		bus = pci_scan_root_bus(pp->dev, sys->busnr, &sunxi_pcie_ops,
					sys, &sys->resources);
	} else {
		bus = NULL;
		BUG();
...
static int sunxi_pcie_map_irq(const struct pci_dev *dev, u8 slot, u8 pin)
{
...
static void sunxi_pcie_add_bus(struct pci_bus *bus)
{
	if (IS_ENABLED(CONFIG_PCI_MSI)) {
		struct pcie_port *pp = sys_to_pcie(bus->sysdata);

		sunxi_pcie_msi_chip.dev = pp->dev;
		bus->msi = &sunxi_pcie_msi_chip;
	}
}

static struct hw_pci sunxi_pci = {
	.setup		= sunxi_pcie_setup,
	.scan		= sunxi_pcie_scan_bus,
	.map_irq	= sunxi_pcie_map_irq,
	.add_bus	= sunxi_pcie_add_bus,
};
...
void sunxi_pcie_setup_rc(struct pcie_port *pp)
{
	...
	sunxi_pcie_readl_rc(pp, PCIE_TYPE1_CLASS_CODE_REV_ID_REG, &val);
	val |= PCI_CLASS_BRIDGE_PCI<<16;
	sunxi_pcie_writel_rc(pp, val, PCIE_TYPE1_CLASS_CODE_REV_ID_REG);
	...
	val &= PCIE_PRIMARY_BUS_MASK;
	val |= PCIE_PRIMARY_BUS_ENABLE;
	sunxi_pcie_writel_rc(pp, val, PCI_PRIMARY_BUS);

	/* setup memory base, memory limit */
	membase = ((u32)pp->mem_base & PCIE_MEMORY_MASK) >> PCIE_HIGH_16;
	memlimit = (config->mem_size + (u32)pp->mem_base) & PCIE_MEMORY_MASK;
