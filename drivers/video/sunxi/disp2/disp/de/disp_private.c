/*
 * linux-3.10/drivers/video/sunxi/disp2/disp/de/disp_private.c
 *
...
static int disp_dma_map_core(int fd, struct dmabuf_item *item)
{
	...
	sg_count = dma_map_sg_attrs(g_disp_drv.dev, sgt_bak->sgl,
...
s32 disp_set_fb_info(struct fb_address_transfer *fb, bool left_eye)
{
	...
	for (i = 0; i < len; ++i) {
		...
		if (fmt_attr_tbl[i].format == fb->format) {
			...
			if (DISP_FORMAT_YUV420_P == fb->format ||
				DISP_FORMAT_YUV420_P_10BIT == fb->format) {
				/* v */
