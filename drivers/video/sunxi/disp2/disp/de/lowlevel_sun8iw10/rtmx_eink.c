/*
 * linux-3.10/drivers/video/sunxi/disp2/disp/de/lowlevel_sun8iw10/rtmx_eink.c
 *
...
void csc_set( int csc_mode, int sel, int csc_en)
{
	...
	for(i=0; i<12; i++)
	{
		de_writel(csc_base +0x10 + (i<<2), csc_mode?csc_tab_601_fullrgbtoyuv[i]:csc_tab_601_yuvtofullrgb[i]);
	}
...
void rt_mixer_set(int sel,unsigned int addr0,unsigned int addr1,unsigned int addr2,unsigned int w,unsigned int h,
				  unsigned int outw,unsigned int outh,unsigned int fmt)
{
	...
	if		(DE_FORMAT_YUV422_I_VYUY	== fmt){r_fmt = 0x0;}
	else if (DE_FORMAT_YUV422_I_YVYU	== fmt){r_fmt = 0x1;}
	else if (DE_FORMAT_YUV422_I_UYVY	== fmt){r_fmt = 0x2;}
	else if (DE_FORMAT_YUV422_I_YUYV	== fmt){r_fmt = 0x3;}
	else if (DE_FORMAT_YUV422_SP_VUVU	== fmt){r_fmt = 0x5;}
	else if (DE_FORMAT_YUV422_SP_UVUV	== fmt){r_fmt = 0x4;}
	else if (DE_FORMAT_YUV422_P			== fmt){r_fmt = 0x6;}
	else if (DE_FORMAT_YUV420_SP_VUVU	== fmt){r_fmt = 0x9;}
	else if (DE_FORMAT_YUV420_SP_UVUV	== fmt){r_fmt = 0x8;}
	else if (DE_FORMAT_YUV420_P			== fmt){r_fmt = 0xa;}
	else if (DE_FORMAT_YUV411_SP_VUVU	== fmt){r_fmt = 0xd;}
	else if (DE_FORMAT_YUV411_SP_UVUV	== fmt){r_fmt = 0xc;}
	else if (DE_FORMAT_YUV411_P			== fmt){r_fmt = 0xe;}
	else									   {uisel = 0x1;r_fmt = fmt;}

	ccnt = 0;
	if(!uisel)
	{
		if		 (r_fmt<=0x3)	  {ycnt = 2;cinw=w>>1; cinh=h;}
		else if	 (r_fmt==0x6)   {ycnt = 1;ccnt=1;cinw=w>>1; cinh=h;}//YUV422
		else if	 (r_fmt<=0x9)   {ycnt = 1;ccnt=0;cinw=w>>1; cinh=h>>1;}
		else if	 (r_fmt==0xa)   {ycnt = 1;ccnt=1;cinw=w>>1; cinh=h>>1;}//YUV420
		else if	 (r_fmt<=0xd)   {ycnt = 1;ccnt=1;cinw=w>>2; cinh=h;}
		else if	 (r_fmt==0xe)   {ycnt = 1;ccnt=2;cinw=w>>2; cinh=h;}//YUV411
		if       ((r_fmt==0x4)||(r_fmt==0x5)){cinw=w>>1; cinh=h;}//YUV422
		csc_set(0,sel,1);//yuv2rgb
	}
	else
	{
		if		 (r_fmt<=0x7)  ycnt = 4;
		else if	 (r_fmt<=0x9)  ycnt = 3;
		else                   ycnt = 2;
		cinw = w;
		cinh = h;
		csc_set(1,sel,0);//rgb2yuv
	}
...
#else
void rt_mixer_init(int sel,unsigned int addr0,unsigned int addr1,unsigned int addr2,unsigned int w,unsigned int h,
				  unsigned int outw,unsigned int outh,unsigned int fmt)
{
	int ycnt,ccnt,uisel;
	int cinw, cinh,r_fmt;
