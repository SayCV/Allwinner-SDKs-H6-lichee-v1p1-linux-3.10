/*
 * linux-3.10/drivers/video/sunxi/disp2/disp/de/disp_display.c
 *
...
s32 disp_device_attached(int disp_mgr, int disp_dev, enum disp_output_type output_type, enum disp_output_type mode)
{
	...
	if (dispdev && dispdev->set_manager) {
			...
			if (dispdev->set_static_config)
				dispdev->set_static_config(dispdev,
							&config);
			return 0;
	}
...
s32 bsp_disp_sync_with_hw(disp_bsp_init_para * para)
{
	if ((1 == para->boot_info.sync)
		...
		config.type = type;
		config.mode = mode;
		/*config.format = (type == DISP_OUTPUT_TYPE_LCD) ?
			DISP_CSC_TYPE_RGB : DISP_CSC_TYPE_YUV444;
		config.bits = DISP_DATA_8BITS;
		config.eotf = DISP_EOTF_GAMMA22;
		config.cs = DISP_BT709;*/
		config.format = para->boot_info.format;
		config.bits = para->boot_info.bits;
		config.eotf = para->boot_info.eotf;
		config.cs = para->boot_info.cs;
...
s32 bsp_disp_hdmi_check_support_mode(u32 disp, enum disp_tv_mode mode)
{
	u32 num_screens = 0;
	s32 ret = 0 ;
