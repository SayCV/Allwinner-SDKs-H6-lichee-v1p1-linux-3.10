    ...
    struct device_attribute *attr, char *buf)
{
  ...
  ssize_t count = 0;
	...
	for (screen_id=0; screen_id < num_screens; screen_id ++) {
		...
		/* capture */
		if (cptr)
			count += cptr->dump(cptr, buf + count);
		/* output */
		if (dispdev->type == DISP_OUTPUT_TYPE_LCD) {
			count += sprintf(buf + count,
				"\tlcd output(%s)\tbacklight(%3d)\tfps:%d.%d",
				(dispdev->is_enabled(dispdev) == 1) ?
				"enable" : "disable",
		...
		if (dispdev->type != DISP_OUTPUT_TYPE_NONE) {
			count += sprintf(buf + count, "\t%4ux%4u\n",
					width, height);
			count += sprintf(buf + count,
					"\terr:%u\tskip:%u\tskip T.O:%u\tirq:%lld\tvsync:%u\tvsync_skip:%u\t\n",
					info.error_cnt, info.skip_cnt,
					info.skip_cnt_timeout,
				...
				struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "disp_para=%x init_disp=%x tv_vdid=%x fb_base=0x%x"
				   " disp_config0=%d,%d - %d,%d,%d,%d disp_config1=%d,%d - %d,%d,%d,%d\n",
...
static DEVICE_ATTR(boot_para, S_IRUGO|S_IWUGO, disp_boot_para_show, NULL);
...
static s32 parser_disp_init_para(const struct device_node *np, disp_init_para * init_para)
{
	...
	if (of_property_read_u32(np, "screen0_output_format", &value) < 0) {
		__wrn("of_property_read screen0_output_format fail\n");
	} else {
	...
	if (of_property_read_u32(np, "screen0_output_bits", &value) < 0) {
		__wrn("of_property_read screen0_output_bits fail\n");
	} else {
	...
	if (of_property_read_u32(np, "screen0_output_eotf", &value) < 0) {
		__wrn("of_property_read screen0_output_eotf fail\n");
	} else {
	...
	if (of_property_read_u32(np, "screen0_output_cs", &value) < 0) {
		__wrn("of_property_read screen0_output_cs fail\n");
	} else {
	...
	if (of_property_read_u32(np, "screen0_output_dvi_hdmi", &value) < 0) {
		__wrn("of_property_read screen0_output_dvi_hdmi fail\n");
	} else {
	...
	if (of_property_read_u32(np, "screen0_output_range", &value) < 0) {
		__wrn("of_property_read screen0_output_range fail\n");
	} else {
	...
	if (of_property_read_u32(np, "screen0_output_scan", &value) < 0) {
		__wrn("of_property_read screen0_output_scan fail\n");
	} else {
	...
	if (of_property_read_u32(np, "screen0_output_aspect_ratio", &value) < 0) {
		__wrn("of_property_read screen0_output_aspect_ratio fail\n");
	} else {
		init_para->output_aspect_ratio[0] = value;
		init_para->using_device_config[0] = true;
	}

	if (!of_property_read_u32_array(np, "disp_config0", tmp, 6)) {
		if (init_para->output_type[0] != tmp[0]) {
			__wrn("output type from boot disp_config0 is not match\n");
		} else {
			init_para->output_format[0] = tmp[2];
			init_para->output_bits[0]   = tmp[3];
			init_para->output_cs[0]     = (tmp[4] != 0) ? tmp[4] : init_para->output_cs[0];
			init_para->output_eotf[0]   = (tmp[5] != 0) ? tmp[5] : init_para->output_eotf[0];
			printk("disp_config0: fmt-0x%04x bits-0x%04x cs-0x%04x eotf-0x%04x\n",
				init_para->output_format[0], init_para->output_bits[0], init_para->output_cs[0], init_para->output_eotf[0]);
		}
	}
	...
	//screen1
		...
		if (of_property_read_u32(np,
			...
			__wrn("of_property_read screen1_output_format fail\n");
		} else {
		...
		if (of_property_read_u32(np,
			...
			__wrn("of_property_read screen1_output_bits fail\n");
		} else {
		...
		if (of_property_read_u32(np,
			...
			__wrn("of_property_read screen1_output_eotf fail\n");
		} else {
		...
		if (of_property_read_u32(np, "screen1_output_cs", &value) < 0) {
			__wrn("of_property_read screen1_output_cs fail\n");
		} else {
			init_para->output_cs[1] = value;
			init_para->using_device_config[1] = true;
		}
		if (of_property_read_u32(np, "screen1_output_dvi_hdmi", &value) < 0) {
			__wrn("of_property_read screen1_output_dvi_hdmi fail\n");
		} else {
		...
		if (of_property_read_u32(np, "screen1_output_range", &value) < 0) {
			__wrn("of_property_read screen1_output_range fail\n");
		} else {
		...
		if (of_property_read_u32(np, "screen1_output_scan", &value) < 0) {
			__wrn("of_property_read screen1_output_scan fail\n");
		} else {
		...
		if (of_property_read_u32(np, "screen1_output_aspect_ratio", &value) < 0) {
			__wrn("of_property_read screen1_output_aspect_ratio fail\n");
		} else {
			init_para->output_aspect_ratio[1] = value;
			init_para->using_device_config[1] = true;
		}
		if (!of_property_read_u32_array(np, "disp_config1", tmp, 6)) {
			if (init_para->output_type[1] != tmp[0]) {
				__wrn("output type from boot disp_config1 is not match\n");
			} else {
				init_para->output_format[1] = tmp[2];
				init_para->output_bits[1]   = tmp[3];
				init_para->output_cs[1]     = (tmp[4] != 0) ? tmp[4] : init_para->output_cs[1];
				init_para->output_eotf[1]   = (tmp[5] != 0) ? tmp[5] : init_para->output_eotf[1];
				printk("disp_config1: fmt-%04x bits-%04x cs-%04x eotf-%04x\n",
					init_para->output_format[1], init_para->output_bits[1], init_para->output_cs[1], init_para->output_eotf[1]);
			}
		}
	}
#if 0
...
static int disp_device_set_config(disp_init_para *init, unsigned int screen_id)
{
...
static void start_work(struct work_struct *work)
{
	...
	int screen_id;
	int count = 0;
	disp_init_para *init = &g_disp_drv.disp_init;
			...
			if (((disp_mode	== DISP_INIT_MODE_SCREEN0) && (screen_id == 0))
				...
				if ((output_type == DISP_OUTPUT_TYPE_LCD)) {
					if (lcd_registered	&& bsp_disp_get_output_type(screen_id) != DISP_OUTPUT_TYPE_LCD) {
						disp_device_set_config(init, screen_id);
...
s32 disp_unregister_sync_finish_proc(void (*proc)(u32))
{
	...
	list_for_each_entry_safe(ptr, tmp,
			&g_disp_drv.sync_finish_proc_list.list, list) {
		if (ptr->proc == proc) {
			list_del(&ptr->list);
			disp_sys_free((void*)ptr);
			return 0;
...
extern void hwc_suspend_prepare(void);
...
long disp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case DISP_EINK_UPDATE:
	{
		...
		ret = bsp_disp_eink_update(eink_manager,
				(struct disp_layer_config *) &lyr_cfg[0],
	...
	case DISP_LAYER_SET_CONFIG:
	{
		...
		const unsigned int lyr_cfg_size =
			sizeof(lyr_cfg) / sizeof(lyr_cfg[0]);
	...
	case DISP_LAYER_SET_CONFIG2:
	{
		unsigned int i = 0;
		const unsigned int lyr_cfg_size =
			sizeof(lyr_cfg2) / sizeof(lyr_cfg2[0]);
		...
		for (i = 0; (i < lyr_cfg_size) && (i < ubuffer[2]); ++i) {
			if (0 == lyr_cfg2[i].enable) {
				memset(&(lyr_cfg2[i].info), 0,
					sizeof(lyr_cfg2[i].info));
			}
		}
	...
	{
		ret = hdmi_cec_send_one_touch_paly();
		break;
	}
#endif
	//----enhance----
