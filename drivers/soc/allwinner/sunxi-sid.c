...
static unsigned int sunxi_soc_mediaid;
...
static void sid_customerid_init(void)
{
	...
	if (sid_get_base(&node, &base, "allwinner,sunxi-chipid", sec))
		return;
	if (vid_align == 0) {
		sunxi_soc_customerid = sid_readl(base + EFUSE_VERDOR_ID_OFFSET,
		sec);
	} else if (vid_align == 2) {
		high_u32 = sid_readl(base + EFUSE_VERDOR_ID_OFFSET - 2, sec);
		low_u32 = sid_readl(base + EFUSE_VERDOR_ID_OFFSET + 2, sec);
		sunxi_soc_customerid = (high_u32 << 16) | (low_u32 >> 16);
	} else {
		SID_ERR("check EFUSE_VERDOR_ID_OFFSET.\n");
		sid_put_base(node, base , sec);
		return;
	}
	sid_put_base(node, base , sec);
	init_flag = 1;
	#else
	SID_ERR("Not support vendor id\n");
	#endif
}

static void sid_mediaid_init(void)
{
	void __iomem *base = NULL;
	struct device_node *node = NULL;
	static s32 init_flag;
	int sec = sunxi_soc_is_secure();
	int offset_shift = EFUSE_MEDIA_ID_OFFSET % 4;
	...
	if (sid_get_base(&node, &base, "allwinner,sunxi-chipid", sec))
		return;
	sunxi_soc_mediaid = sid_readl(
			(base + EFUSE_MEDIA_ID_OFFSET - offset_shift), sec);
	sunxi_soc_mediaid = (sunxi_soc_mediaid >> (8 * offset_shift)) & 0xFF;
	init_flag = 1;
...
unsigned int sunxi_get_soc_mediaid()
{
	sid_mediaid_init();
	return sunxi_soc_mediaid;
}
EXPORT_SYMBOL(sunxi_get_soc_mediaid);
...
{
	s32 ret = sunxi_efuse_readn_ext(key_name, buf, n);
	if (ret < 0)
		return ret;
	return 0;
}
