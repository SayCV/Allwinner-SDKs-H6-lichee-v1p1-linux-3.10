...
void set_pll(void)
{
	...
	if (0 == cpu_id) {
		...
		/* switch to 24M */
		*(volatile __u32 *)(&CmuReg->SysClkDiv) =
		...
		if (unlikely(mem_para_info.debug_mask & PM_STANDBY_ENABLE_JTAG)) {
			...
			if (likely
				...
				*(volatile __u32 *)(AW_JTAG_PF_GPIO_PA) =
				    AW_JTAG_PF_CONFIG_VAL;
			} else {
				*(volatile __u32 *)(AW_JTAG_PH_GPIO_PA) =
		...
		*(volatile __u32 *)(&CmuReg->Pll1Ctl) = (0x00001100) | (0x80000000);	/* N = 17, P=M=1 -> pll1 = 17*24 = 408M */
#endif
		...
		*(volatile __u32 *)(&CmuReg->Cpu_Clk_Src) =
		    CPU_CLK_REST_DEFAULT_VAL;
		...
		*(volatile __u32 *)(&CmuReg->Pll_C0_Cfg) =
		    (CPU_PLL_REST_DEFAULT_VAL) | (0x80000000);
		*(volatile __u32 *)(&CmuReg->Pll_C1_Cfg) =
		    (CPU_PLL_REST_DEFAULT_VAL) | (0x80000000);
	...
	*(volatile __u32 *)(&CmuReg->SysClkDiv) =
	    (0x1000) | (*(volatile __u32 *)(&CmuReg->SysClkDiv));
#else
	*(volatile __u32 *)(&CmuReg->SysClkDiv) =
	    (0x20000) | ((~0x30000) &
			 (*(volatile __u32 *)(&CmuReg->SysClkDiv)));
	...
	for (i = 0; i < (0x40); i += 4)
		*(volatile __u32 *)(SUNXI_GIC_DIST_PBASE + 0x80 + i) =
		    0xffffffff;
	...
	permission = 0 b1111 << 28;	/* secure & non-secure is permitted to access. */
	region_size =
	    (__tzasc_calc_2_power(dram_size * 1024 / 32) + 0 b001110) << 1;
	...
	permission = 0 b1100 << 28;	/* only secure is permitted to access. */
	region_size =
	    (__tzasc_calc_2_power(reserved_dram_size * 1024 / 32) +
	     0 b001110) << 1;
