...
static unsigned long time_to_wakeup;
...
static const extended_standby_manager_t *extended_standby_manager_id;
...
static int suspend_status_flag;
...
static int check_sys_pwr_dm_status(char *pwr_dm)
{
	...
	if (0 == ret) {
		pm_printk(PM_STANDBY_PRINT_PWR_STATUS,
			  KERN_INFO "%s: no child, use by %s, property: sys.\n",
			  ldo_name, pwr_dm);
	} else {
		for (i = 0; i < ret; i++) {
			...
			if ((-1) != is_sys_pwr_dm_id(enable_id)) {
				pm_printk(PM_STANDBY_PRINT_PWR_STATUS,
					  KERN_INFO "property: sys \n");
			} else {
				pm_printk(PM_STANDBY_PRINT_PWR_STATUS,
					  KERN_INFO "property: module \n");
...
static int aw_pm_begin(suspend_state_t state)
{
	...
	if ((FIRST_BOOT_FLAG) != suspend_status
	    && (RESUME_COMPLETE_FLAG) != suspend_status) {
		suspend_result = -1;
...
static int aw_suspend_cpu_die(void)
{
	...
	call_firmware_secure_op(suspend);
#else
...
#define MAX_RETRY_TIMES (5)
	...
	mem_para_info.monitor_vector = call_firmware_secure_op(suspend_prepare);
	call_firmware_secure_op(get_cp15_status,
				virt_to_phys((void *)
					     &(mem_para_info.
					       saved_secure_mmu_state)));
#endif
	printk("hsr: monitor_vector %x\n", mem_para_info.monitor_vector);
	memcpy((void *)phys_to_virt(DRAM_MEM_PARA_INFO_PA),
	       (void *)&mem_para_info, sizeof(mem_para_info));
		...
		memcpy((void *)(phys_to_virt(DRAM_EXTENDED_STANDBY_INFO_PA)),
		       ...
		       sizeof(*
			      (extended_standby_manager_id->
			       pextended_standby)));
	dmac_flush_range((void *)phys_to_virt(DRAM_MEM_PARA_INFO_PA),
			 (void *)(phys_to_virt(DRAM_EXTENDED_STANDBY_INFO_PA) +
				  DRAM_EXTENDED_STANDBY_INFO_SIZE));
	...
	if ((super_standby_para_info.
	     event & (CPUS_WAKEUP_DESCEND | CPUS_WAKEUP_ASCEND)) == 0) {
#ifdef	CONFIG_AW_AXP
		axp_powerkey_set(1);
#endif
	}
	...
	super_standby_para_info.resume_code_src =
	    (unsigned long)(virt_to_phys((void *)&resume1_bin_start));
	...
	if ((NULL != extended_standby_manager_id)
		...
		super_standby_para_info.pextended_standby =
		    (int)(extended_standby_t *) (DRAM_EXTENDED_STANDBY_INFO_PA);
	else
		super_standby_para_info.pextended_standby = (int)NULL;
#endif
	...
	if (unlikely(debug_mask & PM_STANDBY_PRINT_STANDBY)) {
		...
		pr_info
		    ("resume_code_src = 0x%x, resume_code_length = %d. resume_code_length = %x \n",
		     super_standby_para_info.resume_code_src,
		     super_standby_para_info.resume_code_length,
		     super_standby_para_info.resume_code_length);

		pr_info
		    ("total(def & dynamic config) standby wakeup src config: 0x%x.\n",
		     super_standby_para_info.event);
	...
	arisc_standby_super((struct super_standby_para
			     *)(&super_standby_para_info), NULL, NULL);
#endif
...
{
	int i = 0;
	memcpy((void *)&mem_para_info,
	       (void *)phys_to_virt(DRAM_MEM_PARA_INFO_PA),
	...
	if (unlikely(debug_mask & PM_STANDBY_PRINT_RESUME_IO_STATUS)) {
		...
		for (i = 0; i < (GPIO_REG_LENGTH); i++) {
			printk(KERN_INFO "ADDR = %x, value = %x .\n",
			       (volatile __u32)(IO_ADDRESS(AW_PIO_BASE) +
						i * 0x04),
			       *(volatile __u32 *)(IO_ADDRESS(AW_PIO_BASE) +
						   i * 0x04));
		}
...
static void aw_pm_show_dev_status(void)
{
	...
	if (unlikely(debug_mask & PM_STANDBY_PRINT_CPUS_IO_STATUS)) {
		...
		for (i = 0; i < (CPUS_GPIO_REG_LENGTH); i++) {
			printk(KERN_INFO "ADDR = %x, value = %x .\n",
			       (volatile __u32)(IO_ADDRESS(AW_R_PIO_BASE) +
						i * 0x04),
			       *(volatile __u32 *)(IO_ADDRESS(AW_R_PIO_BASE) +
						   i * 0x04));
		}
	...
	if (unlikely(debug_mask & PM_STANDBY_PRINT_CCU_STATUS)) {
		...
		for (i = 0; i < (CCU_REG_LENGTH); i++) {
			printk(KERN_INFO "ADDR = %x, value = %x .\n",
			       (volatile __u32)(IO_ADDRESS(AW_CCM_BASE) +
						i * 0x04),
			       *(volatile __u32 *)(IO_ADDRESS(AW_CCM_BASE) +
						   i * 0x04));
		}
...
static int aw_pm_enter(suspend_state_t state)
{
	...
	if (NORMAL_STANDBY == standby_type) {
		...
		if (PM_SUSPEND_MEM == state || PM_SUSPEND_STANDBY == state) {
			...
			standby_info.standby_para.axp_event =
			    CPUS_BOOTFAST_WAKEUP;
		...
		if ((NULL != extended_standby_manager_id)
			...
			standby_info.standby_para.pextended_standby =
			    (int)(extended_standby_t
			     *) (DRAM_EXTENDED_STANDBY_INFO_PA);
		else
			standby_info.standby_para.pextended_standby = (int)NULL;
	...
	|| defined(CONFIG_ARCH_SUN8IW9P1)) && defined(CONFIG_AW_AXP)
		...
		if ((NULL != extended_standby_manager_id)
		    && (NULL != extended_standby_manager_id->pextended_standby))
			memcpy((void
				*)(phys_to_virt(DRAM_EXTENDED_STANDBY_INFO_PA)),
			       (void *)(extended_standby_manager_id->
					pextended_standby),
			       sizeof(*
				      (extended_standby_manager_id->
				       pextended_standby)));
		...
		PM_DBG
		    ("platform wakeup, normal standby cpu0 wakesource is:0x%x\n, normal standby cpus wakesource is:0x%x. \n",
		...
		arisc_query_wakeup_source((unsigned int
...
static int config_dynamic_standby(void)
{
	...
	if (1 == enable) {
		...
		/*local_standby->soc_pwr_dep.cpux_clk_state.osc_en         |= 0xf;       */
