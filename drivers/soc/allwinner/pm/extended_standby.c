	...
	case 2:		/* PLL3 PLL_Audio = 24M*N/(input_div+1)/(output_div+1)/(P+1) */
		...
		divisor =
		    (standby_data->extended_standby_data.
		     pll_factor[index].divi +
		     1) *
		    (standby_data->extended_standby_data.
		     pll_factor[index].divo +
		     1) *
		    (standby_data->extended_standby_data.pll_factor[index].p +
		     1);
		standby_rate = do_div(dividend, divisor);
...
static int copy_extended_standby_data(scene_extended_standby_t *standby_data)
{
	...
	if (!standby_data) {
		...
		if ((0 != temp_standby_data.id)
		    &&
		    (!((standby_data->
			extended_standby_data.id) & (temp_standby_data.id)))) {
			...
			if (0 != standby_data->extended_standby_data.pll_change) {
					...
					if (standby_data->extended_standby_data.pll_change & (0x1 << i)) {
						if (!
						    (temp_standby_data.pll_change
						     & (0x1 << i)))
							temp_standby_data.pll_factor
							    [i] =
							    standby_data->extended_standby_data.pll_factor
							    [i];
				...
				temp_standby_data.pll_change |=
				    standby_data->
				    extended_standby_data.pll_change;
			}
			if (0 != standby_data->extended_standby_data.bus_change) {
					...
					if (standby_data->extended_standby_data.bus_change & (0x1 << i)) {
						if (!
						    (temp_standby_data.bus_change
						     & (0x1 << i)))
							temp_standby_data.bus_factor
							    [i] =
							    standby_data->extended_standby_data.bus_factor
							    [i];
				...
				temp_standby_data.bus_change |=
				    standby_data->
				    extended_standby_data.bus_change;
			}
...
const extended_standby_manager_t *get_extended_standby_manager(void)
{
	...
	if ((NULL != manager_data)
	    && (NULL != manager_data->pextended_standby))
