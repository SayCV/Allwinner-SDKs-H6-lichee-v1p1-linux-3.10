...
/*	Description:  This file implements basic functions for standby test*/
     /**/
/*	History:*/
/*				2015/08/14		lhk			0.10	Initial version*/
/*				2015/11/09		lhk			0.20	fix basic function*/
/*				2015/12/15		lhk			0.30	fix code style*/
/*				2015/12/15		lhk			0.40	add ddr2 support,fix lpddr2 bug*/
/*******************************************************************************/
#include "mctl_reg-sun8iw10.h"
#include "mctl_hal-sun8iw10.h"
#include "mctl_para-sun8iw10.h"
...
unsigned int ccm_set_pll_ddr0_sscg_standby(__dram_para_t *para)
{
	...
	case 0:
		return 1;
	case 1:
		mctl_write_w((0xccccU | (0x3U << 17) | (0x48U << 20) |
			      (0x3U << 29) | (0x1U << 31)),
			     PLL_DDR0_PAT_CTL_REG);
		break;
	case 2:
		mctl_write_w((0x9999U | (0x3U << 17) | (0x90U << 20) |
			      (0x3U << 29) | (0x1U << 31)),
			     PLL_DDR0_PAT_CTL_REG);
		break;
	case 3:
		mctl_write_w((0x6666U | (0x3U << 17) | (0xD8U << 20) |
			      (0x3U << 29) | (0x1U << 31)),
			     PLL_DDR0_PAT_CTL_REG);
		break;
	case 4:
		mctl_write_w((0x3333U | (0x3 << 17) | (0x120U << 20) |
			      (0x3U << 29) | (0x1U << 31)),
			     PLL_DDR0_PAT_CTL_REG);
		break;
	case 5:
		mctl_write_w(((0x3U << 17) | (0x158U << 20) | (0x3U << 29)
			      | (0x1U << 31)), PLL_DDR0_PAT_CTL_REG);
		break;
	default:
		mctl_write_w((0x3333U | (0x3 << 17) | (0x120U << 20) |
			      (0x3U << 29) | (0x1U << 31)),
...
unsigned int ccm_set_pll_ddr0_clk_standby(__dram_para_t *para)
{
	...
	/*Update PLL Configuration */
	mctl_write_w(rval | (1U << 30), _CCM_PLL_DDR0_REG);
	/*Enable PLL_DDR0 LOCK Function*/
	rval = mctl_read_w(CCU_PLL_LOCK_CTRL_REG);
	rval |= (0x1 << 3);
	mctl_write_w(rval, CCU_PLL_LOCK_CTRL_REG);
	while (!
	       ((mctl_read_w(_CCM_PLL_DDR0_REG) & (1 << 30 | 1 << 28)) ==
		(0 << 30 | 1 << 28)))
		;
	/*Clear Value */
	rval = mctl_read_w(CCU_PLL_LOCK_CTRL_REG);
...
unsigned int ccm_set_pll_ddr1_clk_standby(__dram_para_t *para)
{
	...
	mctl_write_w(rval, CCU_PLL_LOCK_CTRL_REG);
	while (!
	       ((mctl_read_w(_CCM_PLL_DDR1_REG) & (1 << 30 | 1 << 28)) ==
		(0 << 30 | 1 << 28)))
		;
	/*Clear Value */
	rval = mctl_read_w(CCU_PLL_LOCK_CTRL_REG);
...
signed int init_DRAM_standby(int type, __dram_para_t *para)
{
	...
	dram_dbg_0("DRAM STANDBY DRIVE INFO: V0.3\n");
/*****************************************************************************
Function : DRAM Controller Basic Initialization
*****************************************************************************/
	dram_dbg_1("DRAM CLK =%d MHZ\n", para->dram_clk);
	dram_dbg_1("DRAM Type =%d (2:DDR2, 3:DDR3, 6:LPDDR2, 7:LPDDR3)\n",
		   para->dram_type);
	dram_dbg_1("DRAM zq value: 0x%x\n", para->dram_zq);
	ret_val = mctl_core_init_standby(para);
	if (ret_val == 0) {
		dram_dbg_1("DRAM initial error : 1 !\n");
		return 0;
	}
/*****************************************************************************
Function : DRAM size
