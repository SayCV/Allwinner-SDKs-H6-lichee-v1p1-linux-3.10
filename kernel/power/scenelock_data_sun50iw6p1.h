/*
 * kernel/power/scenelock_data_sun50iw6p1.h
...
#ifndef _LINUX_SCENELOCK_DATA_SUN50IW6P1_H
#define _LINUX_SCENELOCK_DATA_SUN50IW6P1_H
...
scene_extended_standby_t extended_standby[] = {
	{
		...
		.soc_pwr_dep.soc_io_state.io_state[0]     = {0x0300b074, 0x0ffff000, 0x07777000},
		.soc_pwr_dep.soc_io_state.io_state[1]     = {0x0300b074, 0x00000000, 0x00000000},
#if 0
		/* for pb port */
	...
	{
		.scene_type     = SCENE_HDMI_CEC_STANDBY,
		.name           = "hdmi_cec_standby",
		.soc_pwr_dep.id                 = HDMI_STANDBY_FLAG,
		.soc_pwr_dep.soc_pwr_dm_state.state = BITMAP(VCC_DRAM_BIT) | \
				BITMAP(VDD_CPUS_BIT) | BITMAP(VCC_LPDDR_BIT) |\
				BITMAP(VCC_PL_BIT)   | BITMAP(VDD_SYS_BIT) | \
				BITMAP(VCC_IO_BIT)   | BITMAP(VCC_PLL_BIT),
		.soc_pwr_dep.soc_pwr_dm_state.volt[0]      = 0,	/* mean: donot need care about the voltage. */
		.soc_pwr_dep.soc_pwr_dm_state.volt[VDD_SYS_BIT]      = 780,
		.soc_pwr_dep.cpux_clk_state.osc_en = BITMAP(OSC_LOSC_BIT) | \
						     BITMAP(OSC_HOSC_BIT) | \
						     BITMAP(OSC_LDO0_BIT) | \
						     BITMAP(OSC_LDO1_BIT),
